...
namespace 相机机械臂VisionPro联合编程
{
    ...
    public class MCCard
    {
        ...
        public void OpenAirPump(bool isOpen)
        {
            ...
            if (isOpen)
            {
                gts.mc.GT_SetDoBit(gts.mc.MC_GPO, 3, 0); //气泵工作(K3)
                gts.mc.GT_SetDoBit(gts.mc.MC_GPO, 4, 1); //吸盘吸物件(K4)
            }
            else
            {
                
                gts.mc.GT_SetDoBit(gts.mc.MC_GPO, 4, 0);// 吸盘放物件(K4)
                gts.mc.GT_SetDoBit(gts.mc.MC_GPO, 3, 1); //气泵关闭(K3)
            }
