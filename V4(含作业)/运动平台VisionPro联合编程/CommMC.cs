...
namespace 相机机械臂VisionPro联合编程
{
    ...
    public class MCCard
    {
        ...
        public bool OpenGTSCardAndInit(string ConfigFileName = GlobalVarAndFunc.cst_default_GTS800_Cfg_FileName)                                                                    //打开控制卡并初始化
        {
            ...
            for (short axis = 1; axis <= GlobalVarAndFunc.cst_MaxAxisNum; axis++)
            {
                ...
                //sRtn += gts.mc.GT_SetPos(axis, 0);             //设置目标位置
                //sRtn += gts.mc.GT_SetVel(axis, 1);             //设置目标速度
